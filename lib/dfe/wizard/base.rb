...
module DfE
  module Wizard
    class Base
      ...
      # @deprecated use state_stores and routing strategies to handle edit
      #
      def edit?
        @edit.present?
      end
      ...
      # @deprecated use state stores to handle data
      #
      def step_params
      ...
      # @deprecated use state store to handle step params
      #
      ...
      # @deprecated use routing strategy
      #
      ...
      def previous_step_path(fallback: nil)
        ...
        if previous_step_klass.present?
          ...
          if route_strategy.present?
            route_strategy.resolve(step: previous_step_name, data:)
          else
            current_step.previous_step_path(previous_step_klass)
          end
        else
          return fallback if steps_processor.present?
      ...
      def next_step_path
        ...
        if next_step_klass.present?
          ...
          if route_strategy.present?
            route_strategy.resolve(step: next_step_name, data:)
          elsif edit?
      ...
      def next_step
        return steps_processor.next_step if steps_processor.present?

        current_step.next_step
      end

      def previous_step
        return steps_processor.previous_step if steps_processor.present?

        current_step.previous_step
      end

      def find_step(step_name)
        steps_list = if steps_mapping.present?
                       steps_mapping
                     else
                       steps
                     end
