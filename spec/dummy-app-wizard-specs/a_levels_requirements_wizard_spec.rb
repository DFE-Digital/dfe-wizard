RSpec.describe ALevelsRequirementsWizard do
  ...
  it 'has the correct operations configured' do
    expect(wizard).to have_step_operations(
      what_a_level_is_required: [
        StepOperations::CreateALevel,
      ],
      remove_a_level_subject_confirmation: [
        StepOperations::RemoveALevelSubjectConfirmation,
      ],
    )
  end
  ...
  describe 'flow path traversal' do
    ...
    context 'at a_level_equivalencies step' do
      ...
      it 'flow path includes full journey' do
        wizard.current_step_name = :course_edit
    ...
    context 'at remove_a_level_subject_confirmation step with no remaining A-levels' do
      it 'exits to course_edit when no A-levels remain' do
        expect(wizard).to branch_from(:remove_a_level_subject_confirmation)
          .to(:course_edit)
          .when(a_level_subject_requirements: [], uuid: '123', confirmation: 'yes')
      end
    end
  end
      ...
      context 'when A-levels remain after deletion' do
        it 'returns to add_a_level_to_a_list' do
          expect(wizard).to branch_from(:remove_a_level_subject_confirmation)
            ...
            .when(uuid: '123', confirmation: 'yes')
        end
      end
    ...
    context 'course_edit redirect step' do
      ...
      it 'resolves to course edit page' do
        expect(wizard).to resolve_step(:course_edit).to(
          url_helpers.recruitment_cycle_provider_course_path(
  ...
  describe '#step' do
    ...
    it 'returns hydrated step object' do
      ...
      expect(step.subjects).to eq([{ uuid: '123', subject: 'maths', minimum_grade_required: 'A' }])
    end
